...
int countIncludes(const string a1[], int n1, const string a2[], int n2)
{
	...
	if (n1 < n2)
		...
		if (a1[0] != a2[0])
			times = countIncludes(a1 + 1, n1 - 1, a2, n2);
		else {
			times += countIncludes(a1 + 1, n1 - 1, a2, n2); //check to see if more instances of this word
			
			times += countIncludes(a1 + 1, n1 - 1, a2 + 1, n2 - 1); //checks for next word
		}
